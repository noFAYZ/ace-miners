...
const MyAce = () => {
  ...
  function stringToIntArray(inputString, limit) {
    try {
        ...
        const intArray = inputString.split(',').map(Number).filter(num => !isNaN(num)).slice(0, limit);
        return intArray;
    } catch (error) {
...
function getRewardsToClaim(claimData, nftIds, dropData) {
  ...
  nftIds.forEach((nftId) => {
      ...
      const dropDetails = dropData.find((drop) => drop.dropId === claimDetails?.dropId);
      ...
      rewardsToClaim.push(rewards);
        ...
        dropData?.forEach((item)=>{
            const isFound = claimDetails.find((claim) => claim.dropId === item?.dropId)
            console.log(isFound)
            if(!isFound){
              rewards.eachLTC += Number(item?.eachLTC)
              rewards.eachCKB += Number(item?.eachCKB)
      ...
      if (claimDetails && dropDetails && !claimDetails.isClaimed && claimDetails.requestStatus === "completed") {
          const rewards = {
...
useEffect(()=>{
  ...
  return (
    <MainLayout>
      <div className="flex justify-center container pt-10 align-middle items-center">
        ...
        <div>
          ...
          <select
            ...
            value={selectedValue}
            onChange={async (e)=>{setSelectedValue(e.target.value)
        ...
        {isChecking ? <>
          <button className="flex gap-2 border rounded px-3 py-1 border-slate-600 hover:bg-slate-800"
      ...
      <CheckerModal isModal={rewardData?.length>0 ? true : false} rewards={rewardData}/>
